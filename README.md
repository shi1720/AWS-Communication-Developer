...
**Current verification:** local workflow, 100 automated tests and infrastructure synthesis pass. AWS account activation is blocking the actual deployment and live-service checks. See the [release status and evidence](docs/STATUS.md).
...
Open **http://127.0.0.1:3279**. Local state persists in the ignored `.data/` directory. Local storage supports one server process; AWS deployments use DynamoDB.
...
CloudFront serves the private S3 application and routes same-origin API requests to HTTP API and Lambda. DynamoDB stores accounts, sessions and bounded workspace snapshots. Bedrock proposes typed intent; application tools alone can allocate stock. AWS CDS carries outbound communications and provider events return through SNS/SQS. EventBridge recovers queued outbox work; CloudWatch and a dead-letter queue expose failures.
...
See the [independent review](docs/REVIEW.md) for findings, corrections and the distinction between tested local behavior and pending cloud evidence.
...
The final submission still requires verified AWS runtime evidence, the creator's recorded video, accurate APN/corporate-email eligibility and a real ACE opportunity ID. The checklist records these explicitly; placeholder IDs and simulated deliveries must never be submitted as real evidence.
