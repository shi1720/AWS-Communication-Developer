...
| Test record             | 128 automated tests and production build passed. Hosted recovery and security evidence: `docs/evidence/firebase-deployment.json`, `docs/evidence/firebase-http-smoke.json` and `docs/evidence/firebase-security-smoke.json`. Zero external sends. |
...
Current checkpoint: https://secondcrate.web.app is a verified public rehearsal served by Firebase Hosting, backed by Cloud Run and the named Firestore database `secondcrate` in `europe-west1`. The owner authorised the permanent Paid-plan upgrade and AWS confirmed success on 22 September 2026. USD 100 in credits remains. Required service access is still blocked while activation is pending. There is no successful AWS deployment, real message send or Bedrock inference to cite yet.
