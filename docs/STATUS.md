...
| Tests | 127 automated tests, TypeScript and production build passed; new tests cover activation diagnostics and confirmed-order follow-ups |
...
| AWS account | Sign-in and identity verification succeed; the owner completed payment verification and the console confirms a saved default payment method. The registration-resume link returns to Console Home. A subsequent API check still reports FREE / NOT_STARTED with deployment services unavailable. The user's Free plan is unchanged |
...
1. Allow AWS activation to complete after the owner's payment verification on 22 September. Identity verification is already marked Verified and the payment method is now saved. The official registration-resume link returns to Console Home without another form, but the 09:47 UTC SES check still returned a missing-service-subscription error. AWS says activation can take up to 24 hours. If access remains blocked after that period, use the account-activation support path; [AWS_SUPPORT_DRAFT.md](AWS_SUPPORT_DRAFT.md) contains a prepared message, not a submitted case. Do not upgrade the plan or repeat payment verification without a concrete provider request.
2. The entrant confirms existing AWS Partner membership. Sign in to that existing Partner Central organisation to create the ACE record. Read-only Partner Central checks returned an empty partner list for this account and rejected ACE listing with INCOMPATIBLE_BENEFIT_AWS_PARTNER_STATE. These results do not establish whether the entrant belongs to a partner through a different organization/account. AWS documents automatic Paid-plan conversion when a Free account joins APN, so do not enroll this account while its owner requires Free.
...
Current evidence: [Firebase deployment](evidence/firebase-deployment.json), [hosted workflow](evidence/firebase-http-smoke.json), [hosted security](evidence/firebase-security-smoke.json), [post-verification AWS status](evidence/aws-post-verification.json), [responsive browser checks](evidence/responsive-browser.json) and [order follow-up regression](evidence/hosted-order-followup.json). The 127-test application release passed [Linux CI](https://github.com/shi1720/AWS-Communication-Developer/actions/runs/35712481012) on commit `a590746`, including infrastructure synthesis, compiled HTTP smoke and the dependency audit.
